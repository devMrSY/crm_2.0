...
export const createUser = async (req, res) => {
  try {
    ...
    let { firstName, lastName, userName, phoneNumber, password, role } = req.body;
    userName = userName.toLowerCase();

    const existingUser = await User.findOne({ where: { userName } });
    if (existingUser) {
      return apiResponseErr(null, false, statusCode.badRequest, 'User already exists', res);
    }

    if (isNaN(phoneNumber)) {
      return apiResponseErr(null, false, statusCode.badRequest, 'Phone Number must be a number', res);
    }
...
export const getUser = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ where: { userId: userId, role: string.User } });
    ...
    return res
      ...
      .json(apiResponseSuccess(user, true, statusCode.success, 'User retrieved successfully'));
  } catch (error) {
    res
      .status(statusCode.internalServerError)
      .send(
...
export const createTransaction = async (req, res) => {
  try {
    ...
    const { companyId, createdForId, manualId, rate, settlementAmt, role, companyName, customerName, admin } = req.body;
    ...
    if (isTransactionExist) {
      ...
      if ((doneBy.maker && doneBy.checker) || (doneBy.maker && role === string.Maker) || (doneBy.checker && role === string.Checker)) {
        throw new CustomError('Request already pending. Please edit from edit panel', null, 409);
      }
    }

    newTransactionObj['transactionId'] = transaction?.transactionId ? transaction?.transactionId : uuidv4()
    ...
    return apiResponseSuccess(transaction, true, statusCode.create, 'Successfully record saved', res);
  } catch (error) {
    return apiResponseErr(
      error.data ?? null,
      false,
...
export const approveTransaction = async (req, res) => {
  try {
    ...
    if (!transaction) {
      return res
        .status(statusCode.badRequest)
        .json(apiResponseErr(null, false, statusCode.badRequest, 'Transaction not found'));
    }
    ...
    if (status === 'approved') {
      ...
      return res
        .status(statusCode.badRequest)
        .json(apiResponseErr(null, false, statusCode.badRequest, 'Invalid status provided'));
    }
    ...
    return res
      .status(statusCode.success)
      .json(apiResponseSuccess(transaction, true, statusCode.success, 'Transaction status updated successfully'));
  } catch (error) {
    res
      .status(statusCode.internalServerError)
      .send(
...
export const updateTransaction = async (req, res) => {
  try {
    const { transactionId, companyId, createdForId, manualId, rate, settlementAmt, role } = req.body;
    ...
    return res
      .status(statusCode.success)
      .json(apiResponseSuccess(transaction, true, statusCode.success, 'Transaction updated successfully'));
  } catch (error) {
    res
      .status(statusCode.internalServerError)
      .send(
